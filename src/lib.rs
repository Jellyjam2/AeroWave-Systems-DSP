...
impl CognitivePayload {
    ...
    fn unpack_from_bridge(&mut self, raw_bytes: Vec<u8>) -> PyResult<bool> {
        ...
        let sentiment = f32::from_le_bytes(raw_bytes[8..12].try_into().expect("Invalid sentiment bytes"));
        let arousal = f32::from_le_bytes(raw_bytes[12..16].try_into().expect("Invalid arousal bytes"));
        let culture_id = u16::from_le_bytes(raw_bytes[16..18].try_into().expect("Invalid culture ID bytes"));
        ...
        while offset + 2 <= raw_bytes.len() {
            let clause = i16::from_le_bytes(raw_bytes[offset..offset+2].try_into().expect("Invalid clause bytes"));
    ...
    fn pack_to_bridge(&self) -> PyResult<Vec<u8>> {
        ...
        let _clause_count = sat_clauses_heapless.len() as u16;
